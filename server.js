...
// Squad mode: public/private rooms, up to 5 pilots, one-vote-per-round
// majority voting streamed to the ESP32-S3 plane.
// Solo mode: a single pilot flies with direct, instant commands — no
// voting round, just a short per-command cooldown so the servo doesn't
// get spammed faster than it can physically move.
...
const SQUAD_MAX_PLAYERS = 5;
const SOLO_MAX_PLAYERS = 1;
const ROUND_MS = 2000; // matches realistic servo response time (squad mode)
const SOLO_COOLDOWN_MS = 250; // minimum gap between direct commands in solo mode
...
function capacityFor(mode) {
  return mode === "solo" ? SOLO_MAX_PLAYERS : SQUAD_MAX_PLAYERS;
}

function createRoom(visibility, name, mode) {
  const code = makeCode();
  const room = {
    code,
    name: name || `Room ${code}`,
    visibility, // 'public' | 'private'
    mode: mode === "solo" ? "solo" : "squad",
    players: new Map(), // ws -> { id, name }
    plane: null, // ws of the connected ESP32-S3 client
    votes: new Map(), // ws -> command  (squad mode: cleared each round, one vote locked per round)
    roundTimer: null,
    roundDeadline: null,
    lastSoloCommandAt: 0,
    lastResult: null,
...
function roomSummary(room) {
  return {
    ...
    maxPlayers: capacityFor(room.mode),
...
function startRoundLoop(room) {
  if (room.mode !== "squad") return; // solo mode has no rounds
  ...
  broadcast(room, { type: "round_start", deadline: room.roundDeadline, roundMs: ROUND_MS });
}
    ...
    switch (msg.type) {
      case "list_public": {
        const publicRooms = Array.from(rooms.values())
          .filter((r) => r.visibility === "public" && r.players.size < capacityFor(r.mode))
          .map(roomSummary);
      ...
      case "create_room": {
        ...
        const mode = msg.mode === "solo" ? "solo" : "squad";
        const room = createRoom(visibility, msg.name, mode);
      ...
      case "join_room": {
        ...
        if (room.players.size >= cap) {
          const label = room.mode === "solo" ? "Solo room already has a pilot." : `Room is full (${cap}/${cap}).`;
          ws.send(JSON.stringify({ type: "error", message: label }));
          return;
        }
        joinRoom(ws, room, msg.playerName);
        break;
      ...
      case "vote": {
        ...
        if (room.mode === "solo") {
          const now = Date.now();
          if (now - room.lastSoloCommandAt < SOLO_COOLDOWN_MS) {
            ws.send(JSON.stringify({ type: "vote_rejected", reason: "cooldown" }));
            return;
          }
          room.lastSoloCommandAt = now;
          room.lastCommand = msg.command;
          if (room.plane && room.plane.readyState === room.plane.OPEN) {
            room.plane.send(JSON.stringify({ type: "command", command: msg.command }));
          }
          broadcast(room, {
            type: "round_result",
            tally: { [msg.command]: 1 },
        ...
        // Squad mode: one command locked per pilot per round.
...
function joinRoom(ws, room, playerName) {
  ...
  if (room.mode === "squad") {
    if (room.roundTimer) {
      // Round already in progress for everyone else — sync this pilot to the
      // existing deadline instead of resetting the timer for the whole room.
      ws.send(JSON.stringify({ type: "round_start", deadline: room.roundDeadline, roundMs: ROUND_MS }));
    } else {
      startRoundLoop(room);
    }
  }
